...
const color_1 = __nccwpck_require__(439);
...
const sendPayload = (url, payload) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (!response.ok) {
        throw new Error(`Failed sending payload to: ${host}, status: ${response.status}`);
    }
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const core_1 = __importDefault(__nccwpck_require__(186));
const color_helper_1 = __nccwpck_require__(325);
const send_payload_helper_1 = __nccwpck_require__(437);
const discord_normalizer_1 = __nccwpck_require__(700);
...
const run = () => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const color = color_helper_1.normalizeColor(core_1.default.getInput('color', { required: true }));
        const fields = JSON.parse(core_1.default.getInput('fields'));
        const buttons = JSON.parse(core_1.default.getInput('buttons'));
        if (discordURL) {
            const discordPayload = discord_normalizer_1.normalizeDiscordPayload(title, text, color, fields, buttons);
            yield send_payload_helper_1.sendPayload(discordURL, discordPayload);
        }
        if (slackURL) {
            const slackPayload = slack_normalizer_1.normalizeSlackPayload(title, text, color, fields, buttons);
            yield send_payload_helper_1.sendPayload(slackURL, slackPayload);
        }
        if (teamsURL) {
            const teamsPayload = teams_normalizer_1.normalizeTeamsPayload(title, text, color, fields, buttons);
            yield send_payload_helper_1.sendPayload(teamsURL, teamsPayload);
        }
    }
    catch (err) {
...
const normalizeDiscordPayload = (title, text, color, fields, buttons) => {
    return {
        ...
        components: [
            {
                type: 1,
                components: buttons.map((button) => (Object.assign({ type: 2, style: 1 }, button))),
            },
        ],
    };
};
